# Quick Rules — Hermes Fast Path Rulebook (v2.3)

<!-- Hermes 배포용 self-contained 룰북. A~J 규칙 본문은 원본 프로젝트 taxonomy의 quick 패턴에서 동기화하고, 실행·검증 지침은 Hermes 도구에 맞게 이 파일에서 관리한다. 런타임에 저장소의 다른 파일은 필요하지 않다. -->

Hermes Agent가 한 번의 Fast Path 실행에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북이다. 원본 프로젝트의 quick 패턴과 처방만 한 줄씩 포함한다.
...
**과윤문 가드:** 변경률 계산과 채택·경고·거부 경계는 상위 `SKILL.md`의 Procedure 6만 따른다. 이 룰북은 패턴 탐지·수정 규칙만 정의하며 변경률을 자가 추정하지 않는다.
...
## 자체검증 체크리스트 (Hermes Fast Path 윤문 후 자가 점검)
...
2. **변경률**: `SKILL.md` Procedure 6의 `execute_code` 계산과 게이트 판정을 확인했는가. 이 룰북 안에서는 값을 자가 추정하지 않는다
...
- **C**: S1 잔존 1~2 또는 자체검증 4항 이하 통과 — 별도 사람 검토 권고
- **D**: S1 잔존 3+ 또는 `SKILL.md` Procedure 6 게이트 거부 — 작업 중단 권고
